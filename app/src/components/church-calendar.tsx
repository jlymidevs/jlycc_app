// app/src/components/church-calendar.tsx
// Shared month calendar (grid + list). Rendered standalone on the public
// /church/calendar page and inside the dashboard shell at /me/calendar —
// `basePath` keeps month/filter navigation on whichever page hosts it.
...
export default async function ChurchCalendar({
  basePath,
  searchParams,
}: {
  basePath: string;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`${basePath}${qs(shiftMonth(year, month, -1))}`}
          ...
          <Link
            href={`${basePath}${qs(currentManilaMonth())}`}
            className="rounded-md border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50"
          >
            Today
          </Link>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {types.map((t) => (
          <Link
            key={t.eventTypeId}
            href={`${basePath}?month=${monthParam}&type=${t.eventTypeId}`}
            className={`rounded-full px-3 py-1 text-xs font-medium ${
