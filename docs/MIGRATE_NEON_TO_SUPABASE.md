# Runbook: Migrate Database Neon PG18 → Supabase (Postgres)

> **For Codex.** Scope is the **database only**. The app stays on **Vercel** — do NOT touch Vercel hosting or CI/CD. **No app-code change is needed**: the app reads `process.env.DATABASE_URL`, so the migration is a dump → restore → env swap. Keep Neon fully intact until the cutover is verified; Neon is the rollback.
...
- Willingness to create a project + copy its connection strings and DB password.
...
After creation, from **Project Settings → Database → Connection string**, copy both:

```
# DIRECT (port 5432) — for restore + Flyway
postgresql://postgres:[PASSWORD]@db.[PROJECT_REF].supabase.co:5432/postgres

# TRANSACTION POOLER (port 6543) — for the Vercel app
postgresql://postgres.[PROJECT_REF]:[PASSWORD]@aws-0-[region].pooler.supabase.com:6543/postgres
```

Export for the shell (fill in real values; never print/commit the password):
...
Take a brand-new full dump (schema + data + `flyway_schema_history`) of live Neon. Use the `postgres:18` Docker image (Neon server is PG18; older `pg_dump` refuses with a version mismatch). Stream to a file to avoid Windows path-mount issues.
...
- If the dump contains `CREATE EXTENSION` lines (e.g. `uuid-ossp`, `pgcrypto`), Supabase supports the common ones; they should restore fine.
...
> `ON_ERROR_STOP=0` lets it continue past harmless "already exists" notices on `public`/`schema` objects that Supabase pre-creates. Review the tail output: real errors (missing extension, failed COPY) must be investigated, not ignored. If a `CREATE EXTENSION` fails, create it manually in the Supabase SQL editor and re-run only the affected part.
...
Optional — confirm Flyway agrees (future migrations will use `$SUPA_DIRECT`):
